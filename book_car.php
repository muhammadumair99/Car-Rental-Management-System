<?php
session_start(); // ✅ Important: Start session at the top
?>
		...
		<ul class="properties_list">
			<?php
				...
				if (isset($_GET['id'])) {
					...
					echo "<script>alert('Car ID not found'); window.location='index.php';</script>";
					exit();
				}
			?>
			<li>
				...
				<div class="property_details">
					<h1><a href="book_car.php?id=<?php echo $rws['car_id'] ?>">Car Make > <?php echo $rws['car_type'];?></a></h1>
			...
			if (isset($_POST['save'])) {
				...
				if ($result === TRUE) {
					...
					echo "<script>alert('Registration Failed. Try Again'); window.location='book_car.php?id=$car_id';</script>";
				}
			}
			?>
		</ul>
